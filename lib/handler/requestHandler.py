...
class cRequestHandler:
    ...
    def __callRequest(self):
        ...
        try:
            ...
            if self.BUG_SSL:
                EXlog('Retrying with SSL bug')
                import ssl
                gcontext = ssl.SSLContext(ssl.PROTOCOL_SSLv23)
                oResponse = urllib.request.urlopen(oRequest, timeout = self.__timeout,context=gcontext)
            else:
                oResponse = urllib.request.urlopen(oRequest, timeout = self.__timeout)
            ...
            if e.code == 503:
                ...
                if cloudflare.CheckIfActive(e.read()):
                    ...
                    EXlog( 'Page protegee par cloudflare')
            ...
            EXlog("(%s),%s" % (e.reason , self.__sUrl))
            return ''           
