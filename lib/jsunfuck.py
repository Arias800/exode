...
class JSUnfuck(object):
    ...
    def repl_words(self, words):
        while True:
            start_js = self.js
            for key, value in sorted(list(words.items()), key=lambda x: len(x[0]), reverse=True):
    ...
    def repl_arrays(self, words):
        for word in sorted(list(words.values()), key=lambda x: len(x), reverse=True):
    ...
    def repl_numbers(self):
        ...
        while True:
            start_js = self.js
            for key, value in sorted(list(self.numbers.items()), key=lambda x: len(x[0]), reverse=True):
                self.js = self.js.replace(key, value)
